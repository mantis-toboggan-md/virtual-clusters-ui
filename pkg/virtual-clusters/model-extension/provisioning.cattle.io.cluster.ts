...
export class VClusterModelExtension implements IClusterModelExtension {
  ...
  useFor(cluster: ICluster) {
    return cluster?.metadata?.annotations[CAPI_ANNOTATIONS.UI_CUSTOM_PROVIDER] === 'k3k';
  }
  ...
  provisionerDisplay(cluster: ICluster): string {
    return cluster?.metadata?.annotations[CAPI_ANNOTATIONS.UI_CUSTOM_PROVIDER];
  }

  parentCluster(cluster: ICluster): string {
    return cluster.metadata?.labels?.[LABELS.PARENT_CLUSTER_DISPLAY] ||
      cluster.metadata?.annotations?.[ANNOTATIONS.PARENT_CLUSTER_DISPLAY];
  }

  async postDelete(cluster: ICluster): Promise<any> {
    const parentClusterId = cluster.metadata?.annotations?.[ANNOTATIONS.PARENT_CLUSTER];
    const namespace = cluster.metadata?.annotations?.[ANNOTATIONS.K3K_NAMESPACE];
